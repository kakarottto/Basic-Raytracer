...
bool refract(const Vec3& v, const Vec3& n, double ni_over_nt, Vec3& refracted){
	Vec3 vnormal = unit_vector(v);
	double dt = dot(vnormal,n);
	double discriminant = 1.0 - ni_over_nt*ni_over_nt*(1-dt*dt);
	if(discriminant>0){
		refracted = ni_over_nt*(vnormal - n*dt)-n*sqrt(discriminant);
		return true;
	}
...
double schlick(double cosine, double ref_indx){
	double r0 = (1-ref_indx)/(1+ref_indx);
	r0 = r0*r0;
	return r0+(1-r0)*(pow(1-cosine,5));
}

typedef Vec3 Point3; //i aint wrong..
//cuz we live in 3D world these are gonna be the default meaasurements
